...
public class EntityParser {
	...
	private static EjbClass currentBean;
	...
	public void parseEjbToXMLMappings() {
		// TODO log start
		Document doc = XMLParserUtils.parseXml(ParserConstants.generatedRepoPath + File.separator + mappingFileName);
		if(doc != null) {
			...
			for (int i=0; i<propertyNodes.getLength(); i++) {
				...
				DataContainer.getInstance().addEntityBean(load(xmlFile));
				// TODO log success
			}
		}else {
			//TODO log error
	...
	private static EjbClass parseEJBClass(Document document) throws ClassNotFoundException {
	...
	private static ColumnAttribute lookupColumnAttribute(String attrName, Class<?> lookupClass) {
		...
		DataContainer appCache = DataContainer.getInstance();
		EjbClass ejb = null;
		...
		try {
			...
			if(lookupClass.getName().equals(currentBean.getEntityClass())) {
				ejb = currentBean;
			}else {
				ejb = parseEJBClass(doc);
			}
